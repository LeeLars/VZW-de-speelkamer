...
// Generate dynamic month filters based on available activities
function generateMonthFilters() {
    ...
    let filtersHTML = `
        <button onclick="filterByMonth('all')" id="filter-all" 
                class="month-filter-btn ${currentMonthFilter === 'all' ? 'bg-sk_teal text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'} px-4 py-2 rounded-full text-sm font-bold transition-all">
            Alle Datums
        </button>
    `;
    ...
    months.forEach(monthKey => {
        ...
        const isActive = currentMonthFilter === monthKey;
        
        filtersHTML += `
            <button onclick="filterByMonth('${monthKey}')" id="filter-${monthKey}" 
                    class="month-filter-btn ${isActive ? 'bg-sk_teal text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'} px-4 py-2 rounded-full text-sm font-bold transition-all capitalize">
                ${monthName} '${shortYear}
            </button>
        `;
    });
...
function filterByMonth(monthStr) {
    ...
    // Regenerate filters with updated active state
...
function renderActivities() {
    ...
    // First, generate the month filters based on available data
    generateMonthFilters();
