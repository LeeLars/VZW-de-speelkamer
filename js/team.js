        ...
        return `
            <div class="group bg-white rounded-[2rem] shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-500 hover:-translate-y-2">
                ...
                <div class="relative overflow-hidden h-72">
                    ...
                    <div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-6 space-y-1">
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-white/20 text-white uppercase tracking-wide">Team</span>
                ...
                <div class="p-6 space-y-4">
                    ${member.intro ? `<p class="text-gray-700 text-sm sm:text-base leading-relaxed font-medium">${member.intro}</p>` : ''}

                    ${member.bio ? `<p class="text-gray-600 text-sm leading-relaxed">${member.bio}</p>` : ''}
                    
                    ${(member.phone || member.email) ? `
                        <div class="pt-3 border-t border-gray-100 grid gap-3">
                            ${member.phone ? `
                                <a href="tel:${member.phone.replace(/\s/g, '')}" class="flex items-center gap-2 text-sm text-gray-700 hover:text-sk_teal transition-colors group">
                                    <span class="w-8 h-8 rounded-full bg-sk_teal/10 text-sk_teal flex items-center justify-center">
