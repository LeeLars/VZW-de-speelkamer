...
function renderTeam() {
    ...
    const createCard = (member, index, variant = 'default') => {
        ...
        const gridLayout = variant === 'featured'
            ? 'grid-cols-1 lg:grid-cols-[auto_minmax(0,1fr)_minmax(0,320px)]'
            : 'grid-cols-1 lg:grid-cols-[auto_minmax(0,1fr)_minmax(0,260px)]';
        ...
        return `
            <div class="group bg-white rounded-[2.5rem] shadow-xl border border-white hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 h-full team-member">
                <div class="grid ${gridLayout} gap-6 lg:gap-10 h-full p-6 sm:p-8 text-center lg:text-left items-start">
                    ...
                    <div class="min-w-0 w-full">
                        <div class="space-y-1 min-w-0">
                            <h3 class="text-2xl font-bold text-gray-900 break-words [overflow-wrap:anywhere]">${member.name}</h3>
                            ...
                            ${member.intro ? `<p data-intro class="mt-4 text-gray-700 text-base leading-relaxed font-medium overflow-hidden transition-[max-height] duration-300 min-w-0 w-full break-words [overflow-wrap:anywhere]" style="max-height: 5rem">${member.intro}</p>` : ''}
                    ...
                    ${(member.phone || member.phone2 || member.email || member.email2) ? `
                        <div class="min-w-0 w-full lg:justify-self-end lg:self-stretch">
                            <div class="pt-6 lg:pt-0 border-t border-dashed border-gray-200 lg:border-t-0 lg:border-l lg:pl-8 lg:h-full">
                                <div class="grid grid-cols-1 gap-3 text-left">
                                    ${member.phone ? `
                                        <a href="tel:${member.phone.replace(/\s/g, '')}" class="flex items-center gap-3 text-sm text-gray-700 hover:text-sk_teal transition-colors group/contact min-w-0">
                                            <span class="w-10 h-10 rounded-2xl bg-sk_teal/10 text-sk_teal flex items-center justify-center flex-shrink-0">
                                    ...
                                    ` : ''}
                                    ${member.phone2 ? `
                                        <a href="tel:${member.phone2.replace(/\s/g, '')}" class="flex items-center gap-3 text-sm text-gray-700 hover:text-sk_teal transition-colors group/contact min-w-0">
                                            <span class="w-10 h-10 rounded-2xl bg-sk_teal/10 text-sk_teal flex items-center justify-center flex-shrink-0">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                                    <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"></path>
                                                </svg>
                                    ...
                                    ` : ''}
                                    ${member.email ? `
                                        <a href="mailto:${member.email}" class="flex items-center gap-3 text-sm text-gray-700 hover:text-sk_teal transition-colors group/contact min-w-0">
                                            ...
                                            <span class="font-semibold min-w-0 w-full max-w-full break-all [overflow-wrap:anywhere] group-hover:underline">${member.email}</span>
                                        </a>
                                    ` : ''}
                                    ${member.email2 ? `
                                        <a href="mailto:${member.email2}" class="flex items-center gap-3 text-sm text-gray-700 hover:text-sk_teal transition-colors group/contact min-w-0">
                                            <span class="w-10 h-10 aspect-square rounded-2xl bg-sk_teal/10 text-sk_teal flex items-center justify-center flex-shrink-0">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                                    <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"></path>
                                                    <polyline points="22,6 12,13 2,6"></polyline>
                                            ...
                                            <span class="font-semibold min-w-0 w-full max-w-full break-all [overflow-wrap:anywhere] group-hover:underline">${member.email2}</span>
                                        </a>
                                    ` : ''}
                                </div>
                            </div>
